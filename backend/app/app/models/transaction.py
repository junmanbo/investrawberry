...
class Transaction(Base):
    ...
    uuid = Column(String)
    order_type = Column(Enum("market", "limit", name="order_type"))
    side = Column(Enum("buy", "sell", name="side"), nullable=False)
    price = Column(Float, nullable=False)
    quantity = Column(Float, nullable=False)
    fee = Column(Float)
    status = Column(Enum("open", "canceled", "closed", name="status"), default="open")
