...
def update_ticker_in_db():
    ...
    try:
        ...
        upbit_exchange = (
            ...
            .filter(models.Exchange.exchange_nm == "UPBIT")
            .first()
        )
...
def update_kr_stock_ticker_in_db():
    db = next(deps.get_db())
    try:
        # My User ID
        user = (
            db.query(models.User)
            .filter(models.User.email == "chchdelm3@icloud.com")
            .first()
        )
        ...
        kis_exchange = (
            db.query(models.Exchange)
            .filter(models.Exchange.exchange_nm == "KIS")
            .first()
        )
